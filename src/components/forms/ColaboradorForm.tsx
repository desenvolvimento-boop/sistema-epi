...
export const ColaboradorForm = ({ onClose, onSaved, initialData }: ColaboradorFormProps) => {
  ...
  const handleFuncaoCreated = async (item: SimpleCrudItem) => {
    try {
      ...
      setSelectedFuncao(String(created.rol_id));
      await reloadRoles();
    } catch (err: any) {
      alert(err.message || 'Erro ao criar função');
    }
  };

  const handleSetorCreated = async (item: SimpleCrudItem) => {
    try {
      const created = await sectionService.create({
        sec_active: item.active ? 1 : 0,
        sec_description: item.name,
        sec_integration_id: item.description,
        usr_id_insert: null,
        usr_id_lastupdate: null,
      });
      setSelectedSetor(String(created.sec_id));
  ...
  const handleEmpresaCreated = async (item: SimpleCrudItem) => {
    try {
      const created = await companyService.create({
        com_active: item.active ? 1 : 0,
        com_description: item.name,
        com_integration_id: item.description,
        usr_id_insert: null,
        usr_id_lastupdate: null,
      });
      setSelectedEmpresa(String(created.com_id));
    ...
    <form className="colaborador-form" onSubmit={handleSubmit}>
      ...
      <div className="colaborador-form-grid">
        <div className="colaborador-form-field">
          ...
          <SimpleCrudModal
            ...
            onClose={() => { setIsFuncaoModalOpen(false); reloadRoles(); }}
            title="Cadastrar Função"
            entityLabel="Função"
            items={funcoes}
            onItemsChange={setFuncoes}
            onItemCreated={handleFuncaoCreated}
          />
        </div>
        ...
        <div className="colaborador-form-field">
          ...
          <SimpleCrudModal
            ...
            onClose={() => { setIsSetorModalOpen(false); reloadSections(); }}
            title="Cadastrar Setor / Seção"
            entityLabel="Setor"
            items={setores}
            onItemsChange={setSetores}
            onItemCreated={handleSetorCreated}
          />
        </div>
      </div>
      ...
      <div className="colaborador-form-grid">
        <div className="colaborador-form-field">
          ...
          <SimpleCrudModal
            ...
            onClose={() => { setIsEmpresaModalOpen(false); reloadCompanies(); }}
            title="Cadastrar Empresa"
            entityLabel="Empresa"
            items={empresas}
            onItemsChange={setEmpresas}
            onItemCreated={handleEmpresaCreated}
          />
        </div>
      ...
      <div className="colaborador-form-actions">
        ...
        >
          {saving ? (
            <>
              <Loader2 className="colaborador-form-add-icon colaborador-form-spin" />
              Salvando...
            </>
          ) : (
